const targets = JSON.parse(process.env.SECURITY_TARGETS_JSON || "[]");
...
const findings = [];
const results = [];
if (!Array.isArray(targets) || !targets.length) throw new Error("SECURITY_TARGETS_JSON must be a JSON array.");
for (const raw of targets) {
  ...
  for (const path of paths) {
    try {
      ...
      results.push({ site, path, error: error.message });
    }
  }
  try {
    ...
    for (const header of required) if (!response.headers.get(header)) findings.push({ fingerprint: `header:${site}:${header}`, site, severity: "medium", title: `Missing security header: ${header}`, route: "/", evidence: {}, impact: "Browser protections are weaker than expected.", recommendation: `Configure the ${header} response header.` });
  } catch {}
}
